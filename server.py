...
def stats():
    con = db()
    rows = con.execute("""
        SELECT player_name, position, COUNT(*) games,
        ...
        FROM player_stats GROUP BY player_name, position ORDER BY player_name, games DESC
    """).fetchall()
    wr = con.execute("""
        SELECT ps.player_name, ps.position,
               SUM(CASE WHEN m.outcome='W' THEN 1 ELSE 0 END) wins,
               COUNT(*) played
        FROM player_stats ps JOIN matches m ON ps.match_id=m.match_id
        GROUP BY ps.player_name, ps.position
    """).fetchall()
    wm = {(r["player_name"], r["position"]): (r["wins"], r["played"]) for r in wr}
    con.close()
    out = []
    for r in rows:
        d = dict(r)
        g = d["games"]
        w, p = wm.get((d["player_name"], d["position"]), (0, max(g,1)))
...
def auto_fetch():
    ...
    sources = [
        ...
        # EA gameType9
        "https://proclubs.ea.com/api/fc/clubs/matches?platform=common-gen5&matchType=gameType9&clubIds=1667112",
    ]
    while True:
        time.sleep(300)
        for url in sources:
            try:
                ...
                with urllib.request.urlopen(req, timeout=15, context=ctx) as r:
                    ...
                    if data:
                        n = store(data)
                        print(f"[AUTO] {n} new match(es) from {url[:50]}")
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port, debug=False)            dribbles_completed INTEGER DEFAULT 0, key_passes INTEGER DEFAULT 0,
            chances_created INTEGER DEFAULT 0, aerial_won INTEGER DEFAULT 0,
            aerial_lost INTEGER DEFAULT 0, headed_goals INTEGER DEFAULT 0,
            sprints INTEGER DEFAULT 0, touches INTEGER DEFAULT 0,
            possession_lost INTEGER DEFAULT 0, yellow_cards INTEGER DEFAULT 0,
